...
function batcDeleteAuditedScripts(){
		if(confirm('确定要删除以选中段子么')){
			$('input[name="selecteds"]:checked').each(function(){
					$.ajax({
						url:baseUrl+'/activity/'+$(this).val(),
						type:'DELETE',
						async:false,
						headers: {
							"Authorization" : "Bearer "+getToken()
...
function getUserList(flag){
	...
	var httpurl = baseUrl+'/users?limit=10'+temp;
...
// 按条件查询用户
function findUser(){
	var query = $('#queryCondition').val();
	if('' ==  query || '用户名或手机号' == query){
		alert("请输入查询参数");
	} else {
		var httpurl = baseUrl+'/users?ql=select * where username = \'' + query + '\' or mobile = \'' + query + '\'&limit=10';
		$.ajax({
			url:httpurl,
			type:'GET',
			headers: {
...
function loadUser(){
	...
	$.ajax({
		...
		success: function(data, textStatus, jqXHR) {
			var d = data.entities;
			for(var i=0;i<d.length;i++){
				var temp = '<option value="'+d[i].path+'">'+d[i].nick+'</option>';
				var tempNick = '<input type="hidden" id="communityNick" value='+d[i].nick+'>';
				$('#communties').append(temp);
				$('#building').append(tempNick);
			}
...
// 获取用户ID
function getUserId(){
	 $.get('http://api.easemob.com:7874/id/users',function(data, status){
		  $('#username').val(data);
	  });
}
...
function saveUser(){
	...
	var d = {
		...
		picture:$('#fileInput').val(),
		password:'123456',
		sex:$("input[name='sex']:checked").val(),
		ispublisher:'1',
		//community:community,
		communityName:$('#communties').val(),
		communityNick:$('#communityNick').val(),
	...
	$.getJSON('http://cloudcode.easemob.com/management/rest/weiquan/linjushuo/ljsusers', d)
...
function batcDeleteUser(){
	...
	window.location.href='yhgl.html?' + subPubPara;
}

//删除用户
function deleteUser(username, flag){
	if (flag){
		$.getJSON('http://cloudcode.easemob.com/management/rest/weiquan/linjushuo/dljsusers', {username:username});
		//$.ajax({
...
function viewUser(){
	$.ajax({
			...
			success: function(d, textStatus, jqXHR) {
				...
				if(d.entities[0].communities && d.entities[0].communities.building)
					$("#building_label").html(d.entities[0].communities.building);
				if(d.entities[0].communities && d.entities[0].communities.communityName)
					$('#communtieName_label').html(d.entities[0].communities.communityName);
