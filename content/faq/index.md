    ...
    <div id="container" class="faq_con">
      ...
      <div class="faq_items">
        <dl>
          ...
          <dd>
            <span></span>
            <p>环信是国内最领先的即时通讯云平台, 只需在手机端加入几行代码就可以帮助APP快捷的接入类似微信的即时通讯功能。比如单聊（发语音，发图片，发位置等)，群聊，实时音频视频等。截止14年9月底，环信已经接入了7000多家APP，注册用户6000多万，过千万用户的超级app有10多家。所以环信既是国内第一家即时通讯云平台，也是国内最大的即时通讯云平台。环信的典型用户包括海豚浏览器（一个亿用户），蜻蜓fm（数千万用户），汽车之家-车友会（数千万用户），节操精选（数百万用户），看动漫（数百万用户），哈图（数百万用户），熊猫公交（数百万用户）等。</p>
          </dd>
          <dt>谁可以使用环信？</dt>
          <dd>
            <span></span>
...
<br>
<p>
环信是一个面向开发者的第三方开发者服务。只要你是有一定技术基础的开发者，你就肯定能很容易自行搞定。环信提供了尽可能完善的文档，demo，就是希望每个开发者都能自助式的用好环信。环信针对大客户是有一些VIP服务，但同时环信也会保证每一个普通的开发者都能得到最有力的支持。这些支持是通过尽可能详尽的文档，尽可能贴近真实需求的demo，尽可能开放的源码，尽可能友好的社区来实现的。</p>
<br>
<p>
环信在商务上的接洽就更简单了。我们的价格体系很简单也完全透明，见：http://www.easemob.com/price/。因为是300万以下注册用户完全免费，所以对绝大多数APP来说，你就直接用就可以了，完全不用去找商务。</p>
