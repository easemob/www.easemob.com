	...
	$.ajax({
		...
		success: function(data, textStatus, jqXHR) {
			...
			for(var i=0;i<d.length;i++){
				...
				var temp = '<tr>'+
				'<td class="text-center"><input type="checkbox" value="'+d[i].name+'" name="selecteds" style="opacity:1;" autocomplete="off"></td>'+
				'<td class="text-center">'+d[i].name+'</td>'+
				'<td class="text-center">'+(d[i].nick ? d[i].nick : "")+'</td>'+				
				'<td class="text-center">'+address+'</td>'+
				'<td class="text-center a-center">'+
				'<a href="xzxq.html?'+param+'&name='+decodeURI(d[i].name)+'" target="_self">编辑</a>'+
				'<a href="javascript:void(0);" onclick="deleteCommuntie(\''+d[i].name+'\');">删除</a>'+	
				'<a href="xqxq.html?'+getPubParam()+'&name='+decodeURI(d[i].name)+'" target="_self">查看</a></td>'+
				'</td>'+
				'</tr>';
				$('tbody').append(temp);
			}
